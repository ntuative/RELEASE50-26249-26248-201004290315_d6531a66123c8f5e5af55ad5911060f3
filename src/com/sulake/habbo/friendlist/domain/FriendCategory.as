package com.sulake.habbo.friendlist.domain
{
   ...
   public class FriendCategory implements IDisposable
   {
      ...
      {
         return Math.ceil(this.var_235.length / const_316);
      }
      ...
      {
         if(this.var_867 >= this.getPageCount())
         {
            this.var_867 = Math.max(0,this.getPageCount() - 1);
         }
      }
      ...
      public function addFriend(param1:Friend) : void
      {
         ...
         var _loc3_:int = 0;
         while(_loc3_ < var_235.length)
         {
            _loc4_ = var_235[_loc3_];
            if(_loc2_ < _loc4_.name.toLowerCase())
            {
               var_235.splice(_loc3_,0,param1);
               return;
            }
            _loc3_++;
         }
      ...
      {
         checkPageIndex();
         return Math.min((this.var_867 + 1) * const_316,this.var_235.length);
      }
