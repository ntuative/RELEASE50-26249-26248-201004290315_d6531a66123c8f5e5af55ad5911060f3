package com.sulake.habbo.catalog.viewer.widgets
{
   ...
   public class MarketPlaceCatalogWidget extends CatalogWidget implements ICatalogWidget, IGetImageListener, IMarketPlaceVisualization
   {
      ...
      private var var_871:MarketPlaceOfferData;
      ...
      private function updateList() : void
      {
         ...
         if(!var_427)
         {
            var_427 = new Timer(25);
            var_427.addEventListener(TimerEvent.TIMER,onPopulationTimer);
         }
         var_872 = 0;
         populateList();
         var_427.start();
      ...
      override public function dispose() : void
      {
         ...
         if(var_427)
         {
            var_427.removeEventListener(TimerEvent.TIMER,onPopulationTimer);
            var_427 = null;
         }
      }
      ...
      private function populateList() : Boolean
      {
         ...
         while(_loc1_ < 5)
         {
            if(var_872 >= var_606.length)
            {
               return true;
            }
            addListItem(var_606.getWithIndex(var_872));
            ++var_872;
            _loc1_++;
         }
         return false;
         ...
         if(param1.type == WindowMouseEvent.WINDOW_EVENT_MOUSE_CLICK)
         {
            switch(param2.name)
            {
               case "back_button":
                  ...
                  break;
               case "buy_button":
                  marketPlace.buyOffer(var_871.offerId);
            }
            return;
         }
